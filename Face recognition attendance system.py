    ...
    with open(ATTENDANCE_CSV, "r+", encoding="utf-8") as f:
        ...
        if name == "UNKNOWN":
            ...
            if name not in nameList:
                f.write(f"{name},{dtString}\n")
                print(f"[LOG] Attendance marked for {name} at {dtString}")
...
class FaceAttendanceApp:
    ...
    def create_login_ui(self):
        ...
        tk.Button(self.master, text="Login", width=12, command=self.check_login).pack(pady=12)
        tk.Label(self.master, text=f"(default: {DEFAULT_USERNAME} / {DEFAULT_PASSWORD})", fg="gray").pack()
    ...
    def create_main_ui(self):
        ...
        for widget in self.master.winfo_children():
            widget.destroy()
        tk.Button(self.master, text="Register Student", width=24, height=2, command=self.open_register_window).pack(pady=8)
        tk.Button(self.master, text="Start Attendance", width=24, height=2, command=self.start_attendance).pack(pady=8)
        tk.Button(self.master, text="View Attendance Sheet", width=24, height=2, command=self.view_attendance).pack(pady=8)
        tk.Button(self.master, text="Reset Attendance", width=24, height=2, command=self.reset_attendance).pack(pady=8)
        tk.Button(self.master, text="Reload Encodings", width=24, height=2, command=self.reload_encodings_ui).pack(pady=8)
        tk.Button(self.master, text="Exit", width=24, height=2, command=self.master.destroy).pack(pady=8)
            ...
            for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
                ...
                if name != "UNKNOWN":
                    ...
                    if not matched.empty:
                        row = matched.iloc[0]
                        display_text = f"{row['student_id']} | {row['name']} | {row['class']} | {row['address']}"
    ...
    def view_attendance(self):
        ...
        with open(ATTENDANCE_CSV, "r", encoding="utf-8") as f:
            ...
            for line in lines:
                ...
                if len(parts) >= 2:
                    name, time = parts[0], ",".join(parts[1:])
                    tree.insert("", "end", values=(name, time))
        tk.Button(top, text="Close", command=top.destroy).pack(pady=5)

    def reset_attendance(self):
        if messagebox.askyesno("Confirm Reset", "Are you sure you want to reset the attendance sheet?"):
            with open(ATTENDANCE_CSV, "w", encoding="utf-8") as f:
                f.write("Name,Time\n")
